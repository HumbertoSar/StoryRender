...
- **`/fio` — trilho do Fio (ativo).** Método McKee reformulado como um fio de 9 degraus conduzido por um tutor socrático. Recomeça do nível 0 da escada: **só texto**, até a instrução estar validada em conversa real. Depois sobe pra (a) canvas/whiteboard com os cartões e (b) open-ended, onde o agente compõe os cartões a partir do contexto e das preferências do autor, sem design system pré-fixado. Rotas: `/fio` é a **tela de seleção de sessão** (lida do checkpointer), `/fio/<thread_id>` é a conversa — o id na URL é o que faz voltar pra ela de outro aparelho —, e `/fio/previa` é o banco de prova de estilo.
